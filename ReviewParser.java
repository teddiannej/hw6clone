...
public class ReviewParser {
	...
	private HashMap<String, Review> parseSemesters(){ 
		...
		String ratingTemplate = "<td class=\"col_r(\\w+)\">(\\d.\\d{2})</td>";

		while(lineIndex < lines.size()){ //Looks for data until the end of the text
			...
			for(int x = 0; x < 12; x++){ //12 possible categories; some semester sections do not select all
				...
				if (ratingCategory.equals("InstructorQuality")){
					instructorQuality = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("CourseQuality")){
					courseQuality = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("Difficulty")){
					difficulty = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("AmountLearned")){
					amountLearned = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("WorkRequired")){
					workRequired = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("ReadingsValue")){
					readingsValue = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("CommAbility")){
					communication = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("InstructorAccess")){
					instructorAccess = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("StimulateInterest")){
					stimulateInterest = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("TAQuality")){
					taQuality = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("RecommendMajor")){
					recommendMajor = Double.parseDouble(findDataPoint(ratingTemplate, 2));
				}
				
				if (ratingCategory.equals("RecommendNonMajor")){
					recommendNonMajor = Double.parseDouble(findDataPoint(ratingTemplate, 2));
